...
class ManageRFQsListSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Expanded(
      child: BlocBuilder<ManageRFQCubit, ManageRfqState>(
        builder: (context, state) {
          return state.whenOrNull(
                failed: (error) {
                  return CustomErrorWidget(
                    errorMessage: error.message ?? 'Unkwon Error',
                  );
                },
                loading: () {
                ...
                success: (rfqs) {
                  if (rfqs.isEmpty) {
                    return const EmptyQuotationWidget();
                  } else {
...
class EmptyQuotationWidget extends StatelessWidget {
  const EmptyQuotationWidget({super.key});
